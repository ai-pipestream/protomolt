  ...
  <v-container fluid class="pa-6">
    <v-card variant="flat" border class="mb-4">
      ...
      <v-card-text class="text-body-2 text-medium-emphasis pt-0">
        A workflow runs serial gRPC calls where each step's request is mapped from the workflow
      ...
      <v-col cols="12" md="3">
        <v-card variant="flat" border>
          <v-card-title class="text-subtitle-2 d-flex align-center">
            Stored
            <v-spacer />
            <v-btn icon="mdi-refresh" variant="text" size="x-small" aria-label="Reload workflows"
                   @click="load" />
          </v-card-title>
        ...
        <v-card variant="flat" border>
          ...
          <v-card-text>
            <p v-if="dirty" class="text-caption text-medium-emphasis mb-2">
              Save the edited definition first — runs execute the stored workflow.
            </p>
            <v-textarea
              v-model="runInput"
              label="Workflow input (proto3 JSON)"
              variant="outlined"
              rows="6"
              auto-grow
              spellcheck="false"
...
const summary = computed(() => (parsed.value ? workflowSummary(parsed.value) : ''))
const inputType = computed(() =>
  parsed.value && typeof parsed.value.inputType === 'string' ? parsed.value.inputType : '')

onMounted(async () => {
  await load()
  // Deep links: ?workflow=<name>&input=<json>&autorun — shareable, reproducible runs.
  const query = route.query
  if (typeof query.workflow === 'string' && names.value.includes(query.workflow)) {
    await select(query.workflow)
  }
...
async function load() {
  try {
    names.value = await listWorkflows()
    error.value = ''
...
async function select(name: string) {
  try {
    const definition = await getWorkflow(name)
    selected.value = name
    saveName.value = name
...
async function save() {
  ...
  try {
    ...
    const gate = e as Error & { findings?: WorkflowFinding[] }
...
async function run() {
  ...
  try {
    ...
    const result = await runWorkflow(selected.value, input)
...
/**
 * The typed viewer needs the output type's descriptor: taken from the workflow schema's
