...
class RecipeWorkbenchMcpTest {
    ...
    void agentCompilesRecordsReplaysAndPromotesTwoLiveServices() throws Exception {
        ...
        try (ProtoMoltServe serve = ProtoMoltServe.start(new ProtoMoltServe.Options(
            ...
            assertThat(mcp.instructions()).contains("compile-recipe", "record-recipe-run",
                    "replay-recipe", "promote-recipe");
            assertThat(mcp.tools()).contains("suggest-mappings", "compile-recipe",
            ...
            JsonNode compiled = mcp.tool("compile-recipe", compile);
            fingerprint = compiled.path("recipeFingerprint").asText();
            ...
            JsonNode evidence = mcp.tool("record-recipe-run", record);
            assertThat(evidence.path("ok").asBoolean()).isTrue();
            assertThat(evidence.path("evidence").path("steps")).hasSize(2);
            assertThat(evidence.path("evidence").path("recipeFingerprint").asText())
            ...
            JsonNode replayed = mcp.tool("replay-recipe", replay);
            ...
            promote.set("recipe", compiled.path("recipe"));
            promote.put("version", "v1");
            JsonNode promoted = mcp.tool("promote-recipe", promote);
            assertThat(promoted.path("promoted").asBoolean()).isTrue();
            assertThat(promoted.path("versionedRecipe").path("recipeFingerprint").asText())
                    .isEqualTo(fingerprint);
        }

        try (GitSchemaRegistryStore store = GitSchemaRegistryStore.builder()
                .repositoryDir(registry).build()) {
            assertThat(new RegistryRecipeRepository(store).find("workbench", "v1"))
                    .isPresent().get().extracting(version -> version.getRecipeFingerprint())
                    .isEqualTo(fingerprint);
        }
        assertThat(directory.resolve("recipes/runs/run-1.pb")).isRegularFile();
    }
    ...
    private ObjectNode chain() {
        ObjectNode chain = MAPPER.createObjectNode();
        chain.put("name", "workbench");
        chain.set("schema", schema());
        chain.put("inputType", "workbench.test.Text");
        chain.put("deadlineMs", 10_000);
        var steps = chain.putArray("steps");
    ...
    private final class McpClient implements AutoCloseable {
        ...
        private McpClient(int port) throws Exception {
            ...
            params.putObject("clientInfo").put("name", "recipe-acceptance").put("version", "0");
