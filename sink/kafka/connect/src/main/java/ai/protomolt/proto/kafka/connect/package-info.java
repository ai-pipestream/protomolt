 ...
 *
 * <p>{@link ai.protomolt.proto.kafka.connect.GrpcSinkConnector} turns records from the subscribed
 * topics into request messages on a configured gRPC method, and
 * {@link ai.protomolt.proto.kafka.connect.GrpcSourceConnector} feeds a topic from a
 * server-streaming method, resuming from a CEL-extracted token stored as the Connect offset. Their
 * configuration lives in {@link ai.protomolt.proto.kafka.connect.GrpcSinkConfig} and
 * {@link ai.protomolt.proto.kafka.connect.GrpcSourceConfig}; the work is done by
 * {@link ai.protomolt.proto.kafka.connect.GrpcSinkTask} and
 * {@link ai.protomolt.proto.kafka.connect.GrpcSourceTask}.</p>
 *
 * <p>The package also carries four transforms that drop into any connector's pipeline, not only
 * these two: {@link ai.protomolt.proto.kafka.connect.ValidateMessage} checks a record value
 * against the rules its schema declares,
 * {@link ai.protomolt.proto.kafka.connect.MapMessage} reshapes it with field-mapping and CEL
 * rules, {@link ai.protomolt.proto.kafka.connect.RedactMessage} masks fields by their declared
 * sensitivity class, and {@link ai.protomolt.proto.kafka.connect.CelFilter} keeps or drops
