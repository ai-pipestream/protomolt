...
import ai.protomolt.proto.search.index.protobuf.ProtobufIndexer;
 ...
 * registers ProtoMolt's metadata, indexing-hint, and validation extensions so declared
 * options are readable rather than unknown fields — without the hint registration every
 * {@code (index)} option would silently fall back to inference.
 */
final class ConnectDescriptors {
    ...
    static List<FileDescriptor> linkedFiles(String descriptorSetBase64) {
        ...
        try {
            ...
            ProtobufIndexer.registerExtensions(extensions);
    ...
    private static Descriptor findMessage(FileDescriptor file, String fullName) {
        ...
        while (start <= relative.length()) {
            int dot = relative.indexOf('.', start);
            String segment = dot < 0 ? relative.substring(start) : relative.substring(start, dot);
            current = current == null
                    ? file.findMessageTypeByName(segment)
                    : current.findNestedTypeByName(segment);
            if (current == null || dot < 0) {
                return current;
            }
            start = dot + 1;
        }
