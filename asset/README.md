...
of the platform consumes. Identification is bounded: it reads a leading
and a trailing window off the pass that already hashes the content, looks
inside containers to tell an office document from an archive, and falls
back to the wider published signature set for evidence about assets the
registry cannot place. The design of record is
...
| `asset/characterize` | `:protomolt-asset-characterize` | The engine: the byte windows an asset is identified from and their streaming capture, the shared media-type sniffer, format grammars compiled from the contract's own expressions (descriptor-parity tested), the container readers for ZIP and compound files, the compiler and matcher for published signature definitions, the declared-versus-identified compatibility relation, and the state machine's one resolution point |
