/**
 * The search door: the user-facing query surface over indexed documents and
 * the indexing RPC durable workflows drive.
 *
 * <p>The door serves a fixed set of <em>mapping subjects</em>
 * ({@link ai.pipestream.proto.search.door.ServedMapping}): each names an
 * index mapping (the queryable surface), the document identity, and
 * optionally a chunk lane whose
 * {@link ai.pipestream.proto.index.spi.ChunkingPolicy} derives per-chunk
 * vectors at index time through the embedding lane. Every request is gated
 * by membership — unknown subjects, fields outside the mapping, and vector
 * queries against subjects without a policy are refused loudly by name.
 *
 * <p>{@link ai.pipestream.proto.search.door.SearchDoorServices} wires the
 * stack; {@link ai.pipestream.proto.search.door.SearchDoorModule} mounts it
 * as the {@code search} role over a co-mounted or remote {@code repo} role.
 * {@link ai.pipestream.proto.search.door.RepoDocumentMapping} is the
 * out-of-the-box subject over repository documents.
 */
package ai.pipestream.proto.search.door;
