...
/**
 * The door as a composed role: a fake repo role serves one document, the
 * search role mounts over its channel, and the whole loop — index through
 * the workflow-facing RPC, query back — runs over the door's real TCP port.
 */
class SearchDoorModuleTest {
