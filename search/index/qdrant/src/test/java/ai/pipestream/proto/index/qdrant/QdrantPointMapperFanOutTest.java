...
/**
 * Fan-out edge cases for the Qdrant payload: a plan path under a repeated ancestor
...
class QdrantPointMapperFanOutTest {
    ...
    void fanOutScalarsBecomeOneListPayloadValue() throws Exception {
        ...
        IndexingPlan plan = plan(
                new IndexingPlan.IndexedField("blob_bag.blobs.blob.filename", "filenames",
                        ResolvedFieldHint.of(IndexFieldKind.KEYWORD), true),
                new IndexingPlan.IndexedField("blob_bag.blobs.blob.size_bytes", "sizes",
    ...
    void anElementWithoutTheLeafOnlyDropsItsOwnValue() throws Exception {
        ...
        IndexingPlan plan = plan(new IndexingPlan.IndexedField(
                "blob_bag.blobs.blob.filename", "filenames",
                ResolvedFieldHint.of(IndexFieldKind.KEYWORD), true));

        ListValue filenames = mapper.map(document, plan).get(0)
    ...
    void fanOutOfValuesWithNoPayloadShapeIsSkippedNotAnEmptyList() throws Exception {
        ...
        Document singular = Document.newBuilder(document())
                ...
                .build();
        assertThat(mapper.map(singular, plan(new IndexingPlan.IndexedField(
        ...
        assertThat(mapper.map(document, plan).get(0).getPayloadMap())
                .doesNotContainKey("blob_metadata");
    }

    private static IndexingPlan plan(IndexingPlan.IndexedField... fields) {
        return new IndexingPlan(Document.getDescriptor().getFullName(), List.of(fields));
    }
