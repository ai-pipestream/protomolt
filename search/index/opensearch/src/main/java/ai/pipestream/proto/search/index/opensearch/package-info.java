 ...
 * {@link ai.pipestream.proto.search.index.spi.SearchEngineIndexer} and turns a message plus an
 * {@link ai.pipestream.proto.search.index.spi.IndexMapping} into a {@code Map}, leaving serialization
 * and transport to the caller's client. {@link OpenSearchIndexerProvider} is the
 * {@code ServiceLoader} entry registered under
 * {@link ai.pipestream.proto.search.index.spi.SearchEngineIndexerProvider}.
 *
 * <p>{@link OpenSearchMappingGenerator} derives the index {@code mappings} body from the same
 * mapping, covering field types, sub-fields, doc values, date formats, range types, and
 ...
 * index from the properties generated for a mapping (with {@code index.knn} when the mapping has a vector
 * field) and writes document maps through the {@code _bulk} endpoint, surfacing per-item
 * failures. Engine-agnostic NDJSON emission stays in the separate {@code protomolt-search-index-ndjson}
 * module, which does not interpret the mapping.
 *
