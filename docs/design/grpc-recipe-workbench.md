...
| Typed inter-step dataflow and structured grounding | **LANDED: #100** | Extend the recipe and chain edge contract, not the provider coordinator. An edge selects recipe input or prior outputs, applies existing descriptor-checked mappings and CEL, projects the consumer-visible form, validates it, and then passes that typed value to a gRPC or structured step. Add bounded fan-out with stable branch identities and explicit collect; do not add provider, container, or GPU requirements. | In-process tests map and project one upstream result into a structured step, prove excluded fields never reach its grounding packet, reject an invalid projected value before provider invocation, fan out two items with deterministic identities and bounded concurrency, exercise branch failure policy and collect, and replay the edge fingerprints, verdicts, and cardinality fully offline. Every persisted field has validation and sensitivity metadata; only actual searchable fields receive index annotations. |
| Live structured-inference acceptance | **LANDED: #101** | Exercise one explicitly configured compatible model through MCP and typed gRPC after the grounding edge is available. This package owns test configuration only, not model deployment. | A live opt-in test maps, projects, and validates a prior typed result into one grounded generation through both MCP and typed gRPC, returns a valid protobuf message with replayable evidence, and proves excluded and sensitive values are absent from provider requests and stored evidence; forced repair stays with the scripted provider while the live endpoint owns transport, grounding, validation, and privacy. |
| Provider credential references and authenticated transport | **LANDED: #103** | Add the opaque `credential_ref` catalog field (validation bounds plus secret sensitivity metadata; never credential material), the credential-resolver SPI with the production `env:` resolver, bearer authentication in the shared OpenAI-compatible transport, the launcher spec segment, and the live acceptance's authenticated path. Do not add arbitrary user-controlled header maps, TLS customization, or provider-specific auth schemes. | In-process wire tests prove the resolved bearer token reaches both `/v1` and `/v3` unary and streaming requests, an unresolvable reference fails before any HTTP request is made, unauthenticated entries send no authorization header, and neither the reference nor the resolved secret enters request bodies or failure messages; the opt-in live test passes against a real authenticated endpoint. |
| Authenticated service-profile reflection | **DELEGATABLE** | Extend the service-workspace host boundary so an endpoint's existing opaque `credential_ref` can resolve to host-owned gRPC `CallCredentials`, initially `env:` bearer credentials only. Apply credentials to registration, refresh, inspection probes, and invocation without adding arbitrary metadata maps or persisting material. Keep this independent of the Phase 4 pipeline contract. | In-process authenticated reflection tests prove a valid reference registers and refreshes a profile, a missing reference fails before opening a channel, unauthenticated endpoints remain header-free, and neither reference names nor resolved material enter descriptor artifacts, profile summaries, logs, or errors. A container acceptance self-registers the token-protected coordinator through its TLS route. |
...
#### Phase 4 starting point and delegation
...
| Work package | Status | Ownership boundary | Acceptance evidence |
| --- | --- | --- | --- |
| Agent delegation contract and offline lifecycle checker | **PR OPEN: #109** | The engine-neutral coordinator/worker bidirectional-streaming contract (hello and admission, offer/accept/reject, a single active lease with heartbeat, renewal, and expiry, monotonic progress and resumable checkpoints, the attempt-bound completion-candidate/revision-requested/accepted review flow, and cancellation) plus the pure in-process reducer that checks a recorded transcript and never repairs it. No persistence, Kafka, MCP actions, provider adapters, or process launching. | In-process tests prove happy-path completion, reject/re-offer, checkpoint resume after reconnect, lease expiry and reassignment, the cancellation race, revision and re-review, idempotent and conflicting duplicates, missing acceptance evidence that cannot drive acceptance, stale worker/attempt/revision rejection, non-mutating checkpoint regression, and terminal immutability; dogfood tests prove the contract's own validate.v1 rules evaluate. |
