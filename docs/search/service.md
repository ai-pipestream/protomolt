...
The service serves a fixed set of *mapping subjects* (`ServedMapping`): each
...
both servers mount the gRPC validating interceptor, so a request that
...
service mounts (`EmbeddingProviders.forSpec`), so a subject naming an absent
...
exactly what the durable pipeline has processed. The service fetches
...
Every repo read the service makes (the indexing fetch and replay's listing)
...
node commits and uploads, readers restore and serve. A read-only mount
(`SearchServiceConfig.readOnly`) mounts only the query surface, needs no
...
(`SearchServiceConfig.refreshSeconds`) follows the writer live: each tick
...
`SearchServiceModule.Config` takes an `IndexSnapshots` directly.
...
then the service indexes it under the request's mapping subject
...
every request. The response reports `chunks_deleted`, counted across
...
purge over the default two-phase tombstone), then the service un-indexes it.
...
it rides the repo channel the service already requires, so it needs no parse
coordinator.

## Replay

`replay-documents` (contributed by the service when `jobs` co-mounts) re-runs
...
is a different digest and a different chunk generation, and the service's
...
`protomolt-search-console` is the product page over the service: one pure-JDK
HTTP server, one page, no build step (the playground idiom). The page's
subject and lane pickers are populated from the service's `ListSubjects` RPC,
so what the page offers is exactly what the service serves, and refusals
render verbatim — the service writes them for humans. The server
bridges `POST /search` (proto3-JSON `SearchRequest` in, hits out) and
`GET /subjects` onto the service's gRPC surface, and proxies
