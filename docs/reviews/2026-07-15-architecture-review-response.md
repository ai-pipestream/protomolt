...
| 23 | Join/shape docs described unimplemented behavior | **Fixed.** `docs/transform/join-shapes.md` now states the implemented joiner semantics exactly and marks richer unmatched-entry policies as future options. |
| 24 | Encryption + vector indexing threat model | **Already documented** (docs/search/indexing.md names the neighborhood and inversion channels explicitly and treats vectors as confidential material). Policy checks that refuse unsafe sensitive-field vectorization without explicit authorization are deferred. |
