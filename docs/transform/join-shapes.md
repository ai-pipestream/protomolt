# Joins, unions, and derived shapes

ProtoMolt resolves multiple named messages into declared or generated output
types. The feature includes multi-source mapping, shape synthesis,
`synthesize-shape`, `join-messages`, `merge-schemas`, `infer-schema`, and
bounded stream joins.
...
3. **Emit.** One move produces the merged proto source (registrable, true
   ...
   the new type. The **defined join** is one ruleset reading every source
   at once (coalesced singular fields let later sources overwrite,
   absent values skip; coalesced repeated fields accumulate). The
   **defined union** is one ruleset per source, mapping it alone onto the
...
`StreamJoiner` supports zip and keyed joins over two flow-controlled
`DynamicGrpcStream` instances. Matching is symmetric and immediate: each
arrival probes the other side's buffer. Completed joins wait in their own
queue, so a caller taking few results does not strand a matched pair.

Per-side buffers are bounded and evict the oldest unmatched entry by arrival
order. Duplicate keys queue in FIFO order. An unreadable key is dropped because
it cannot match. Construction validates that both key paths exist, are singular
scalars, and have the same type. Output uses the standard scoped mapping rules.

## Planned extensions

- Declare identity keys in protobuf metadata so joins and connectors share one
  key definition.
- Register a synthesized shape and its source references in one operation.
- Add explicit unmatched-entry policies beyond bounded eviction.
