...
Sensitivity is declared once on the schema field. Every
...
[verb](../surface/actions.md) and as the `RedactMessage`
[Kafka Connect transform](../sink/kafka-connect.md).
...
unresolved payloads are not reported. The [CLI](../apps/cli.md) and the
[MCP server](../surface/mcp.md) pass the JSON envelope through unchanged and have both.
...
change. The transform uses its configured descriptor set as the default
`Any` resolver and does not surface unresolved payloads.
...
Descriptor `json_name` does not survive every descriptor round-trip because
some encoders drop it. `FieldMeta` therefore carries a `json_name`, set by
