...
A chain is a checked serial composition of gRPC and structured-generation
steps. It maps the chain input and prior step outputs into each request, applies
gates and validation, and returns one typed result.

Chains are bounded sidecar operations. Durable scheduling, checkpoint storage,
...
- the chain name, descriptor files, input type, and deadline;
...
supplies its response. Synchronous `run-chain` rejects external steps because
...
`ChainVerifier` checks the full definition without contacting a service:

- every method and protobuf type resolves;
- gRPC chain methods are unary;
...
`check-chain` exposes the verifier through the action catalog. Registry writes
use the same verification gate.

## Execution

`ChainRunner` executes steps in order and fails at the first unrecoverable
error. The whole-chain deadline bounds every step; a step deadline is limited
by the time remaining in the chain.
...
`ChainRepository`; the Git registry provides the standard repository
implementation.

## Recording and promotion

`ChainRecipeCompiler` converts a checked chain to a portable `GrpcRecipe`.
The recipe workbench records redacted fixtures and run evidence, replays them
offline, and promotes immutable versions to the registry.

See [recipes and run evidence](recipes.md).
