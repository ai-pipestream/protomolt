# Recipes and run evidence

A recipe turns a successful gRPC exploration into a checked, replayable
...
[`grpc_recipe.proto`](../../surface/grpc/recipe/src/main/proto/ai/pipestream/proto/grpc/recipe/v1/grpc_recipe.proto).
...
3. check a chain;
4. compile the chain into a recipe;
5. record a live run;
6. replay the fixtures offline; and
7. promote the recipe to the Git registry.

Descriptors, validation rules, fingerprints, and recorded evidence determine
whether the recipe is valid. Model output and suggested mappings do not bypass
those checks.
...
`ChainRecipeCompiler` converts an existing checked `ChainDefinition` without
...
1. select the recipe input or prior step outputs;
...
`env:` reference. The host resolves it immediately before transport. Recipes,
provider request bodies, evidence, logs, and errors do not contain credential
material.

## Run evidence

`RecipeRunRecorder` stores the information needed to review and replay a run:

- recipe, service-profile, descriptor, prompt, and schema fingerprints;
...
`RecipeReplay` performs no network or inference calls. It:
...
- replays mixed gRPC and structured-generation recipes from fixtures.
...
`RegistryRecipeRepository` stores immutable recipe versions in the Git-backed
...
| `check-chain` | Verify methods, types, mappings, CEL, and deadlines |
| `compile-recipe` | Convert a checked chain into a recipe |
| `record-recipe-run` | Execute and persist redacted fixtures and evidence |
| `replay-recipe` | Verify a recorded run offline |
| `promote-recipe` | Store an immutable recipe version in the registry |

The MCP initialize response describes this workflow, and MCP resources expose
service profiles, method contracts, recipes, runs, and artifacts without
