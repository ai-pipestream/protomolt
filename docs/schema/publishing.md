...
- **Compatibility is the registry's.** A server-side compatibility or
  validity rejection surfaces as a `FAILED` outcome with the registry's
  message. The remaining files still publish. Only
...
[Building and testing](../operations/building.md) for running them.
