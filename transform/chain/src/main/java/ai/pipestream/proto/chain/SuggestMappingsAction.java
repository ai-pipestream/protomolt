    ...
    public ObjectNode execute(ObjectNode input, ActionContext context) throws ActionException {
        ...
        for (int index = 0; index < array.size(); index++) {
            if (!(array.get(index) instanceof ObjectNode source)) {
                throw RecipeActionJson.invalid("each source must be an object",
                        "/sources/" + index);
            }
            ...
            Descriptor descriptor = SchemaResolver.resolve(source, "schema", context)
                    .message(RecipeActionJson.optionalText(source, "type"),
                            "/sources/" + index + "/type");
            if (sources.putIfAbsent(name, descriptor) != null) {
                throw RecipeActionJson.invalid("duplicate source name '" + name + "'",
                        "/sources/" + index + "/name");
            }
        }
        ObjectNode targetNode = RecipeActionJson.object(input, "target");
        Descriptor target = SchemaResolver.resolve(targetNode, "schema", context)
                .message(RecipeActionJson.optionalText(targetNode, "type"), "/target/type");
