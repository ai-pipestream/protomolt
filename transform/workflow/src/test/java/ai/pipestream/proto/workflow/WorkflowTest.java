...
/**
 * The workflow end to end against live in-process services: a two-step tokenize-then-embed
...
class WorkflowTest {
    ...
    static void start() throws Exception {
        ...
        file = compiled.descriptorFor("workflow/test/workflow.proto").orElseThrow();
    ...
    private static CompiledWorkflow definition(String when, CompiledWorkflow.Output output) {
        List<FileDescriptor> files = List.of(file);
        return new CompiledWorkflow("embed-text", files,
                file.findMessageTypeByName("Text"), 10_000,
                List.of(CompiledWorkflow.Step.grpc("tokenize", "in-process", false,
                                CompiledWorkflow.resolveMethod(files, "workflow.test.Tokenizer/Tokenize"),
                                null, List.of("text = input.text"), List.of(), false, 0, ""),
                        CompiledWorkflow.Step.grpc("embed", "in-process", false,
                                CompiledWorkflow.resolveMethod(files, "workflow.test.Embedder/Embed"),
    ...
    @Test
    void verifierPassesTheGoodWorkflowAndCatchesScopeErrors() {
        assertThat(new WorkflowVerifier().verify(definition(null, null))).isEmpty();

        // 'embed' cannot be referenced before it runs, and gates must be boolean.
        List<FileDescriptor> files = List.of(file);
        CompiledWorkflow broken = new CompiledWorkflow("broken", files,
    ...
    @Test
    void workflowComposesTwoServicesAndProjectsTheOutput() throws Exception {
        CompiledWorkflow.Output output = new CompiledWorkflow.Output(
                ...
                List.of());
        WorkflowRunner.Result result = runner.run(definition(null, output), text("hi", false));
        ...
        assertThat(result.steps()).extracting(WorkflowRunner.StepOutcome::name)
                .containsExactly("tokenize", "embed");
    }

    @Test
    void falseGateSkipsAStepAndTheWorkflowContinues() throws Exception {
        WorkflowRunner.Result result = runner.run(
                definition("!input.skip_embed", null), text("hi", true));
        // 'embed' skipped: the workflow's output is the last executed response (Tokens).
    ...
    void skippedStepsBindDefaultsSoLaterReferencesStayWellDefined() throws Exception {
        ...
        CompiledWorkflow.Output output = new CompiledWorkflow.Output(
                ...
                List.of("source_text = input.text", "vector = embed.values"),
                List.of());
        CompiledWorkflow workflow = definition("!input.skip_embed", output);
        assertThat(new WorkflowVerifier().verify(workflow)).isEmpty();

        WorkflowRunner.Result result = runner.run(workflow, text("hi", true));
        Descriptor embedding = file.findMessageTypeByName("Embedding");
        assertThat(result.output().getField(embedding.findFieldByName("source_text")))
                .isEqualTo("hi");
        @SuppressWarnings("unchecked")
        ...
        try {
            org.assertj.core.api.Assertions.assertThatThrownBy(() ->
                    ...
                    .satisfies(e -> assertThat(
                            ((WorkflowRunner.WorkflowExecutionException) e).step())
                            .isEqualTo("embed"));
        } finally {
            fail.set(false);
        }
    ...
    @Test
    void channelPolicyCapacityFailuresBecomeRetryableWorkflowFailures() {
        WorkflowRunner rejecting = new WorkflowRunner(step -> {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->
                ...
                .satisfies(error -> {
                    WorkflowRunner.WorkflowExecutionException failure =
                            (WorkflowRunner.WorkflowExecutionException) error;
                    assertThat(failure.kind()).isEqualTo(WorkflowRunner.FailureKind.GRPC);
                ...
                {"workflow": {
                   ...
                   "steps": [
                     ...
                     {"name": "embed", "target": "in-process",
                      "method": "workflow.test.Embedder/Embed",
                      "rules": ["ids = tokenize.ids"]}
                   ],
                   "output": {"type": "workflow.test.Embedding",
