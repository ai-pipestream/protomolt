...
/**
 * A resolved workflow: serial gRPC calls whose requests are mapped from the workflow scope —
 ...
 *
 * @param deadlineMs the whole workflow's budget; per-step deadlines nest inside it
...
public record CompiledWorkflow(String name, List<FileDescriptor> files, Descriptor inputType,
     ...
     *        External steps only execute as jobs; synchronous {@code run-workflow} rejects
    ...
    public record Step(String name, String target, boolean tls, MethodDescriptor method,
        ...
        /**
         * A gRPC step: one unary call whose request is mapped from the workflow scope.
         ...
         * {@link #STRUCTURED_DEPENDENCY}; the workflow's files must carry the target
    ...
    /** The workflow's output shape and the scoped rules that populate it. */
    ...
    /** Resolves {@code package.Service/Method} across the workflow's files. */
    public static MethodDescriptor resolveMethod(List<FileDescriptor> files, String qualified) {
        ...
        throw new IllegalArgumentException(
                "method '" + qualified + "' not found in the workflow's schema");
    }
}
