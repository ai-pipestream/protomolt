...
/**
 * The account store's persistence bootstrap and transaction wrapper: HikariCP
 * pool → Flyway migration, in that order, then plain JDBC.
 * <p>
 * Why plain JDBC where the repo ledger uses JPA: the account schema is two
 * flat tables with no relationships, no lazy loading and no entity graph —
 * the mapping layer would be ceremony. What stays from the repo pattern is
 * the shape that matters: the pool is sized to what the database can serve
 * (virtual threads make the pool, not the thread count, the concurrency
 * limit), Flyway owns the schema, and every unit of work is one short
 * blocking transaction on a virtual thread.
 * <p>
 ...
 * {@link RuntimeException}s (including {@link AccountStoreException}s such
 * as constraint conflicts) propagate intact, checked {@link SQLException}s
 * are wrapped with the original as cause. Nothing is swallowed, and the
...
public final class AccountDatabase implements AutoCloseable {
     ...
     */
    public AccountDatabase(AccountStoreConfig config) {
        HikariConfig hikari = new HikariConfig();
        hikari.setPoolName("account-store");
    ...
    public <T> T inTransaction(Function<Connection, T> work) {
        try (Connection c = dataSource.getConnection()) {
            ...
            try {
                ...
                rollbackQuietly(c);
                throw AccountStoreException.wrap("transactional work failed", e);
            }
        } catch (SQLException e) {
            throw AccountStoreException.wrap("could not borrow a connection", e);
        }
    }
    ...
    public <T> T readOnly(Function<Connection, T> work) {
        try (Connection c = dataSource.getConnection()) {
            ...
            throw AccountStoreException.wrap("read-only work failed", e);
        }
    }
