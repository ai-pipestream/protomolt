...
/**
 * The chain-jobs store's persistence bootstrap and transaction wrapper:
 ...
 * {@link RuntimeException}s (including {@link ChainJobStoreException}s)
 ...
 * returns to the pool.
 */
public final class ChainJobDatabase implements AutoCloseable {
     ...
     */
    public ChainJobDatabase(ChainJobStoreConfig config) {
        HikariConfig hikari = new HikariConfig();
        hikari.setPoolName("chain-jobs-store");
    ...
    public <T> T inTransaction(Function<Connection, T> work) {
        try (Connection c = dataSource.getConnection()) {
            ...
            try {
                ...
                rollbackQuietly(c);
                throw ChainJobStoreException.wrap("transactional work failed", e);
            }
        } catch (SQLException e) {
            throw ChainJobStoreException.wrap("could not borrow a connection", e);
        }
    }
    ...
    public <T> T readOnly(Function<Connection, T> work) {
        try (Connection c = dataSource.getConnection()) {
            ...
            throw ChainJobStoreException.wrap("read-only work failed", e);
        }
    }
