...
/**
 * The outbox relay: drains {@code chain_job_events_outbox} to the
 * chain-job-events Kafka topic. One virtual-thread loop (see {@link #start})
 ...
 * delivery, with {@code ChainJobEvent.event_id} as the consumer dedupe key.
 * A failure increments attempts; at {@link ChainJobEventRecord#MAX_ATTEMPTS}
 ...
 */
public final class ChainJobEventRelay implements AutoCloseable {

    private static final Logger LOG = LoggerFactory.getLogger(ChainJobEventRelay.class);
     ...
     * @param topic the chain-job-events topic
     * @param pollInterval idle backoff between empty drains
     * @param batchSize the claim batch size per drain
     */
    public ChainJobEventRelay(ChainJobStore store, Producer<String, Message> producer,
     ...
     */
    public void start() {
        thread = Thread.ofVirtual().name("chain-job-event-relay").start(() -> {
            while (!closed) {
                try {
                    if (relayOnce() == 0) {
                    ...
                    LOG.warn("chain-job-event-relay iteration failed (loop continues): {}",
                            e.getMessage(), e);
                    sleep(1000);
                }
            }
    ...
    public int relayOnce() {
        List<ChainJobEventRecord> batch = store.pollPendingEvents(batchSize);
        int published = 0;
        for (ChainJobEventRecord record : batch) {
            try {
                ChainJobEvent event = ChainJobEvent.parseFrom(record.payload);
     ...
     * pinned to the ChainJobEvent type (one type, one subject).
     * <p>
