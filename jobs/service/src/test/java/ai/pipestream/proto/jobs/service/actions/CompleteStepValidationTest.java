...
/**
 * complete-step's declared-validation verdict (the {@link ValidatingChains}
 ...
 * checkpoints and requeues. The single-step chain parks immediately, so no
...
class CompleteStepValidationTest {
    ...
    void fresh() {
        ...
        worker = new ChainJobWorker(store, context, null,
                new ChainRunner(step -> InProcessChannelBuilder.forName("unused").build()),
    ...
    /** Submit the external-review chain and park it on the review step. */
    private String parkOnReview() throws Exception {
        ObjectNode request = MAPPER.createObjectNode();
        request.set("chain", chains.externalReviewChain("in-process"));
    ...
    void aResponseFailingTheDeclaredRulesFailsTheJobAsAVerdict() throws Exception {
        ...
        assertThat(rejected.get("status").asText()).isEqualTo(ChainJobRecord.STATUS_FAILED);
        ...
        assertThat(store.events().stream().map(e -> e.eventType))
                .contains(ChainJobEventRecord.TYPE_FAILED);
    }
    ...
    void aResponsePassingTheDeclaredRulesResumesTheJob() throws Exception {
        ...
        assertThat(accepted.get("status").asText()).isEqualTo(ChainJobRecord.STATUS_QUEUED);

        // The resumed job maps the review into the output and completes.
        assertThat(worker.workOnce()).isTrue();
        ChainJobRecord done = store.get(UUID.fromString(jobId)).orElseThrow();
