...
/**
 * The unit/IT chain fixture: one runtime-compiled proto (a tokenize →
 * review → embed pipeline), served as dynamic in-process or localhost-TCP
 * gRPC services, plus builders for the chain-definition JSON envelopes the
 * tests submit. The Embedder fails UNAVAILABLE when its request carries
 * {@code fail = true} (propagated from the chain input through the
...
public final class TestChains {
     ...
     * @return the runner
     */
    public ChainRunner inProcessRunner(String inProcessName) {
        return new ChainRunner(step -> InProcessChannelBuilder.forName(inProcessName).build());
    }

    /** The production runner (channels from the step target), for the TCP IT. */
    public ChainRunner tcpRunner() {
        return new ChainRunner();
    }

    /**
     * The two-step chain envelope: tokenize → embed, output Embedding.
     *
     * @param target the step target
     * @param gate an optional {@code when} gate for the embed step, or null
     * @return the chain-definition JSON
     */
    public ObjectNode twoStepChain(String target, String gate) {
        ObjectNode chain = envelope(target);
        ArrayNode steps = (ArrayNode) chain.get("steps");
        if (gate != null) {
            ((ObjectNode) steps.get(1)).put("when", gate);
        }
        ((ObjectNode) chain.get("output"))
                .putArray("rules").add("source_text = input.text").add("vector = embed.values");
        return chain;
    }

    /**
     * The three-step chain envelope: tokenize → review (external) → embed,
     * output Embedding including the review's notes.
     *
     * @param target the step target
     * @return the chain-definition JSON
     */
    public ObjectNode threeStepChain(String target) {
        ObjectNode chain = envelope(target);
        ArrayNode steps = (ArrayNode) chain.get("steps");
    ...
    /**
     * A chain whose embed rule references a scope that does not exist — it
     * parses but does not verify.
     *
     * @param target the step target
     * @return the broken chain-definition JSON
     */
    public ObjectNode brokenChain(String target) {
        ObjectNode chain = envelope(target);
        ArrayNode rules = (ArrayNode) ((ObjectNode) chain.get("steps").get(1)).get("rules");
        rules.removeAll();
    ...
    private ObjectNode envelope(String target) {
        ...
        chain.putObject("output").put("type", "jobs.test.Embedding");
        return chain;
    }
