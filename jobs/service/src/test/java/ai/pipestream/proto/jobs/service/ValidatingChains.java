 ...
 * tests use.
 */
public final class ValidatingChains {
    ...
    /**
     * The one-step validating chain: tokenize (validate: true) → Out.tag.
     *
     * @param target the step target
     * @return the chain-definition JSON
     */
    public ObjectNode validatingTokenizeChain(String target) {
        ObjectNode chain = envelope("guarded-tokenize", target);
        ...
        ((ObjectNode) chain.get("output")).putArray("rules").add("tag = tokenize.tag");
        return chain;
    }

    /**
     * The one-step external validating chain: review (external, validate:
     * true) → Out.notes. The job parks before anything would call the desk.
     *
     * @param target the step target
     * @return the chain-definition JSON
     */
    public ObjectNode externalReviewChain(String target) {
        ObjectNode chain = envelope("guarded-review", target);
        ArrayNode steps = chain.putArray("steps");
        ...
        ((ObjectNode) chain.get("output")).putArray("rules").add("notes = review.notes");
        return chain;
    }
    ...
    private ObjectNode envelope(String name, String target) {
        ...
        chain.put("inputType", "jobs.guard.Text");
        chain.putObject("output").put("type", "jobs.guard.Out");
        return chain;
    }

    private static String resource(String path) {
        try (InputStream in = ValidatingChains.class.getClassLoader()
