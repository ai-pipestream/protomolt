...
| `surface/grpc/` | `grpc-invoke`, `grpc-service-profile`, `grpc-service-workspace`, `grpc-service`, `grpc-validation`, `grpc-validation-micrometer` | Dynamic gRPC invocation and server reflection; durable service profiles and content-addressed descriptor artifacts; actions for registering, listing, inspecting, and refreshing services; the catalog as a typed gRPC service; validating interceptors with a Micrometer binding |
