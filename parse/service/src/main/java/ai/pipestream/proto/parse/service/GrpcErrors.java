 ...
 * contract, plus the coordinator's single place for exception→status mapping.
 *
 * <p>Mapping policy: explicit status exceptions pass through unchanged — the
 * handlers raise INVALID_ARGUMENT at the decision point, and a repo-service
 * failure arrives as the repo call's own {@link StatusRuntimeException} and
 * is forwarded as-is (the coordinator adds no translation layer over repo's
 * wire contract). {@link IllegalArgumentException} → INVALID_ARGUMENT.
 * Anything else → INTERNAL with only the exception message — never a stack
 * trace — in the status description.
 */
